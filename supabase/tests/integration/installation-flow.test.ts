...
Deno.test('Integration: Complete installation flow from install to fork creation', async () => {
  ...
  try {
    ...
    // Step 5: Verify session was established
...
Deno.test('Integration: Installation flow handles existing fork gracefully', async () => {
  const saved = saveEnvState([
    'GITHUB_CLIENT_ID',
    'GITHUB_APP_ID',
    'GITHUB_PRIVATE_KEY',
