...
Deno.test(
  "handleHealthCheck - returns healthy status for GET request",
  async () => {
...
Deno.test(
  "handleHealthCheck - returns method not allowed for non-GET requests",
  async () => {
    const req = createMockRequest({
      method: "POST",
      url: "https://example.com/health",
...
Deno.test(
  "handleHealthCheck - returns method not allowed for PUT request",
  async () => {
    const req = createMockRequest({
      method: "PUT",
      url: "https://example.com/health",
...
Deno.test(
  "handleHealthCheck - returns method not allowed for DELETE request",
  async () => {
    const req = createMockRequest({
      method: "DELETE",
      url: "https://example.com/health",
...
Deno.test("handleHealthCheck - includes CORS headers in response", () => {
  ...
  // CORS headers should be present (using getCorsHeaders with default config)
  assert(corsOrigin !== null, "Access-Control-Allow-Origin should be present");
  assert(
    corsMethods !== null,
  ...
  // Default CORS config allows wildcard origin and all standard methods
  assertEquals(corsOrigin, "*");
  assertEquals(corsMethods, "GET, POST, PUT, DELETE, OPTIONS");
});
