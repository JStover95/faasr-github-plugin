...
Deno.test("formatErrorMessage - rate limit error", () => {
  ...
  assertEquals(message, "Too many requests. Please try again in a few minutes.");
});
...
Deno.test("formatErrorMessage - permission error", () => {
  ...
  assertEquals(message, "Permission denied. Please check your GitHub App permissions.");
});
...
Deno.test("formatErrorMessage - not found error", () => {
  ...
  assertEquals(message, "Resource not found. Please check your repository and workflow configuration.");
});
...
Deno.test("handleGitHubError - non-Error returns GitHub error message", () => {
  ...
  assertEquals(message, "An unexpected error occurred while communicating with GitHub.");
});
...
Deno.test("createErrorResponse - includes CORS headers", () => {
  ...
  assertEquals(headers.get("Access-Control-Allow-Origin"), corsHeaders["Access-Control-Allow-Origin"]);
  assertEquals(headers.get("Content-Type"), "application/json");
});

Deno.test("createErrorResponse - includes formatted error message", async () => {
  const error = new Error("rate limit exceeded");
  const response = createErrorResponse(error, 429);

  const body = await response.json();
  assertEquals(body.success, false);
  assertEquals(body.error, "Too many requests. Please try again in a few minutes.");
});

Deno.test("createErrorResponse - uses default message for non-Error", async () => {
...
Deno.test("createErrorResponse - uses default message when not provided", async () => {
...
Deno.test("createAuthErrorResponse - includes authentication error message", async () => {
  const response = createAuthErrorResponse();
...
Deno.test("createValidationErrorResponse - includes error message", async () => {
...
Deno.test("createValidationErrorResponse - includes details when provided", async () => {
  const details = ["Error 1", "Error 2"];
  const response = createValidationErrorResponse("Validation failed", details);

  const body = await response.json();
  assertEquals(body.details, details);
});

Deno.test("createValidationErrorResponse - excludes details when not provided", async () => {
  const response = createValidationErrorResponse("Validation failed");
...
Deno.test("createValidationErrorResponse - excludes details when null", async () => {
...
Deno.test("createNotFoundErrorResponse - includes CORS headers", () => {
  const response = createNotFoundErrorResponse("Not found");
...
Deno.test("createConfigurationErrorResponse - includes error message", async () => {
  const response = createConfigurationErrorResponse("GitHub App configuration missing");
